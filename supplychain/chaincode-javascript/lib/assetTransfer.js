...
class IdentityContract extends Contract {
    ...
    async createDID(ctx, did, publicKey, metadata, x509Cert, role, organisation) {
        ...
        console.log(`Document DID créé avec succès: ${did}`);
        return `Document DID créé avec succès pour: ${did}`;
    }
    ...
    async readDID(ctx, did) {
        ...
        console.log(`Document DID récupéré avec succès: ${did}`);
...
validateX509Cert(cert) {
        try {
            // Convertir le certificat PEM en un objet forge
            const certObj = forge.pki.certificateFromAsn1(forge.asn1.fromDer(forge.util.decode64(cert)));

            // Vérifier la date de validité
            const now = new Date();
            if (certObj.validity.notBefore > now || certObj.validity.notAfter < now) {
                throw new Error('Certificat X.509 expiré ou pas encore valide.');
            }

            // Vérifier la signature
            const publicKey = forge.pki.publicKeyFromAsn1(certObj.publicKey);
            const verified = forge.pki.verifyCertificateChain(
                forge.pki.createCaStore(), // Vous pouvez spécifier votre propre store
                [certObj],
                (cert, valid) => valid // La fonction de vérification
            );

            if (!verified) {
                throw new Error('Échec de la vérification de la signature du certificat X.509.');
            }

            // Ici, vous pouvez ajouter des vérifications supplémentaires, comme la révocation

            return true; // Retourne vrai si le certificat est valide
        } catch (err) {
            console.error('Erreur de validation du certificat X.509:', err);
